...
SERVICES := vault gitea gitlab nexus api jenkins nginx qdrant ollama chatbot n8n zammad
START_ORDER := $(SERVICES)
STOP_ORDER := zammad n8n chatbot ollama qdrant nginx jenkins api nexus gitlab gitea vault
DEVOPS_ORDER := vault nexus api gitea jenkins nginx qdrant ollama chatbot n8n zammad
DEVOPS_STOP_ORDER := zammad n8n chatbot ollama qdrant nginx jenkins gitea api nexus vault
DISTCLEAN_ORDER := zammad n8n chatbot ollama qdrant nginx jenkins api nexus gitlab gitea vault
...
help:
	...
	@echo "  make devops MODE=docker|bare   # start vault,nexus,api,gitea,jenkins,nginx,qdrant,ollama,chatbot,n8n,zammad"
	@echo "  make devops-down MODE=docker|bare # stop zammad,n8n,chatbot,ollama,qdrant,nginx,jenkins,gitea,api,nexus,vault"
