...
GITEA_EXAMPLE_REPO=actions-hello-world
...
- `make up` also ensures an example repository (`actions-hello-world`) exists for `myuser` and contains `.gitea/workflows/hello-world.yml`.
- `make up` also ensures a private repository (`jenkins-example`) exists for `myuser` with branch-specific `Jenkinsfile` content:
  - default branch (`main`/`master`): `hello prod world`
  - `dev` branch: `hello dev world`
