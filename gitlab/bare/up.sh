...
BARE_INSTALL_GITLAB="${BARE_INSTALL_GITLAB:-1}"
BARE_INSTALL_RUNNER="${BARE_INSTALL_RUNNER:-1}"
BARE_RUNNER_EXECUTOR="${BARE_RUNNER_EXECUTOR:-shell}"
...
GITLAB_ADMIN_USER="${GITLAB_ADMIN_USER:-admin}"
GITLAB_ADMIN_PASSWORD="${GITLAB_ADMIN_PASSWORD:-password}"
GITLAB_USER="${GITLAB_USER:-user}"
GITLAB_USER_PASSWORD="${GITLAB_USER_PASSWORD:-password}"
...
create_bootstrap_pat() {
  ...
  until [[ "${attempt}" -ge "${max_attempts}" ]]; do
    attempt=$((attempt + 1))
    token="$(
      require_sudo gitlab-rails runner "${script}" 2>/dev/null | tail -n 1 | tr -d '\r' || true
    )"
...
fetch_registration_token() {
  require_sudo gitlab-rails runner 'token = Gitlab::CurrentSettings.current_application_settings.runners_registration_token; puts token.to_s' | tail -n 1
}
...
ensure_root_password() {
  if [[ -z "${GITLAB_ROOT_PASSWORD:-}" || "${GITLAB_ROOT_PASSWORD}" == "${ROOT_PASSWORD_PLACEHOLDER}" ]]; then
    ...
    set_env_var "GITLAB_ROOT_PASSWORD" "${GITLAB_ROOT_PASSWORD}"
...
ensure_managed_users() {
  ...
  until [[ "${attempt}" -ge "${max_attempts}" ]]; do
    ...
    output="$(
      require_sudo env \
        BOOTSTRAP_ADMIN_USER="${GITLAB_ADMIN_USER}" \
        BOOTSTRAP_ADMIN_PASSWORD="${GITLAB_ADMIN_PASSWORD}" \
        BOOTSTRAP_REGULAR_USER="${GITLAB_USER}" \
        BOOTSTRAP_REGULAR_PASSWORD="${GITLAB_USER_PASSWORD}" \
        gitlab-rails runner "${runner_script}" 2>/dev/null | tr -d '\r' || true
    )"
...
install_gitlab() {
  local url="${GITLAB_EXTERNAL_URL:-http://localhost}"
  echo "Installing GitLab CE for external_url=${url}"

  require_sudo apt-get update
  require_sudo apt-get install -y curl ca-certificates openssh-server tzdata perl
  curl -fsSL https://packages.gitlab.com/install/repositories/gitlab/gitlab-ce/script.deb.sh | require_sudo bash
  require_sudo env EXTERNAL_URL="${url}" GITLAB_ROOT_PASSWORD="${GITLAB_ROOT_PASSWORD}" apt-get install -y gitlab-ce
  require_sudo gitlab-ctl reconfigure
  require_sudo gitlab-ctl status
}

install_runner() {
  echo "Installing gitlab-runner"
  curl -fsSL https://packages.gitlab.com/install/repositories/runner/gitlab-runner/script.deb.sh | require_sudo bash
  require_sudo apt-get install -y gitlab-runner
}
...
ensure_runner_credentials() {
  ...
  local api_url="${RUNNER_GITLAB_URL:-${GITLAB_EXTERNAL_URL:-http://localhost}}"
  local pat
  local runner1_token
  local runner2_token
  local registration_token

  wait_for_gitlab_http "${GITLAB_EXTERNAL_URL:-http://localhost}"

  echo "Generating runner credentials automatically"
  pat="$(create_bootstrap_pat 12 5 || true)"

  if [[ -n "${pat}" ]] \
    && runner1_token="$(create_runner_token "${pat}" "bare-worker-1" "bare,worker-1,${BARE_RUNNER_EXECUTOR}" "${api_url}")" \
    && runner2_token="$(create_runner_token "${pat}" "bare-worker-2" "bare,worker-2,${BARE_RUNNER_EXECUTOR}" "${api_url}")"; then
    RUNNER_1_TOKEN="${runner1_token}"
    RUNNER_2_TOKEN="${runner2_token}"
    set_env_var "RUNNER_1_TOKEN" "${RUNNER_1_TOKEN}"
    set_env_var "RUNNER_2_TOKEN" "${RUNNER_2_TOKEN}"
    GENERATED_RUNNER_1_TOKEN="${RUNNER_1_TOKEN}"
    GENERATED_RUNNER_2_TOKEN="${RUNNER_2_TOKEN}"
    ...
    if [[ -n "${registration_token}" ]]; then
      ...
      set_env_var "RUNNER_REGISTRATION_TOKEN" "${RUNNER_REGISTRATION_TOKEN}"
      GENERATED_REGISTRATION_TOKEN="${RUNNER_REGISTRATION_TOKEN}"
...
register_runner_if_missing() {
  local runner_name="$1"
  local runner_tags="$2"
  local runner_token="$3"

  if require_sudo grep -q "name = \"${runner_name}\"" /etc/gitlab-runner/config.toml 2>/dev/null; then
  ...
  local base_args=(
    --non-interactive
    --url "${RUNNER_GITLAB_URL:-${GITLAB_EXTERNAL_URL:-http://localhost}}"
    --executor "${BARE_RUNNER_EXECUTOR}"
  ...
  if [[ -n "${runner_token}" ]]; then
    require_sudo gitlab-runner register "${base_args[@]}" --token "${runner_token}"
  elif [[ -n "${RUNNER_REGISTRATION_TOKEN:-}" ]]; then
    require_sudo gitlab-runner register "${base_args[@]}" --registration-token "${RUNNER_REGISTRATION_TOKEN}"
  else
    echo "No runner auth configured. Set RUNNER_1_TOKEN/RUNNER_2_TOKEN or RUNNER_REGISTRATION_TOKEN in ${ENV_FILE}."
    exit 1
  fi
}
...
print_credentials() {
  echo ""
  echo "GitLab login"
  echo "  URL: ${GITLAB_EXTERNAL_URL:-http://localhost}"
  echo "  Admin username: ${LOGIN_ADMIN_USERNAME}"
  echo "  Admin password: ${GITLAB_ADMIN_PASSWORD}"
  echo "  User username: ${LOGIN_REGULAR_USERNAME}"
  echo "  User password: ${GITLAB_USER_PASSWORD}"
...
if [[ "${BARE_INSTALL_GITLAB}" == "1" ]]; then
  install_gitlab
else
  echo "Skipping GitLab CE install (BARE_INSTALL_GITLAB=${BARE_INSTALL_GITLAB})"
fi

ensure_managed_users

if [[ "${BARE_INSTALL_RUNNER}" == "1" ]]; then
  install_runner
  ensure_runner_credentials
  register_runner_if_missing "bare-worker-1" "bare,worker-1,${BARE_RUNNER_EXECUTOR}" "${RUNNER_1_TOKEN:-}"
  register_runner_if_missing "bare-worker-2" "bare,worker-2,${BARE_RUNNER_EXECUTOR}" "${RUNNER_2_TOKEN:-}"
  require_sudo systemctl enable --now gitlab-runner
  require_sudo systemctl status gitlab-runner --no-pager
else
  echo "Skipping gitlab-runner install (BARE_INSTALL_RUNNER=${BARE_INSTALL_RUNNER})"
fi

sync_credentials_to_vault

echo "Bare mode setup complete"
print_credentials
